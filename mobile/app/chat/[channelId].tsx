...
export default function ChatScreen() {
  ...
  const renderMessage = ({
    ...
    const isFirstInGroup =
      !messageAbove || messageAbove.user?.id !== message.user?.id;
    const isLastInGroup =
      !messageBelow || messageBelow.user?.id !== message.user?.id;

    const showTimestamp = shouldShowTimestamp(message, messageAbove);
    const showAvatar = isLastInGroup;

    const getBubbleStyle = () => {
      let style = "rounded-2xl"; // Base style with large rounding
      ...
      if (isFromCurrentUser) {
        if (isFirstInGroup && isLastInGroup) {
          ...
          // Top message in a group
          style += " rounded-br-md";
        } else if (isLastInGroup) {
          // Bottom message in a group
          style += " rounded-tr-md";
        } else {
            ...
            >
              {sending ? (
                ...
                <Ionicons
                  ...
                  size={20}
                  color={
                    newMessage.trim() && !sending ? "white" : "gray"
                  }
                />
              )}
            </TouchableOpacity>
