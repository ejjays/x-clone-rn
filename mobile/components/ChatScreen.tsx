  ...
  useEffect(() => {
    ...
    const initializeChannel = async () => {
      try {
        ...
        const otherMember = members.find((member: any) => member.user.id !== currentUser.id)
        if (otherMember) {
  ...
  const renderMessage = ({ item: message }: { item: any }) => {
    ...
    return (
      <View className={`flex-row mb-4 ${isFromCurrentUser ? "justify-end" : "justify-start"}`}>
        {!isFromCurrentUser && (
          <Image
            source={{ uri: message.user?.image || `https://getstream.io/random_png/?name=${message.user?.name}` }}
        ...
        {isFromCurrentUser && (
          <Image
            source={{ uri: currentUser.imageUrl || `https://getstream.io/random_png/?name=${currentUser.firstName}` }}
        ...
        {otherUser && (
          <>
            ...
            <View className="flex-1">
              <Text className="font-semibold text-gray-900 text-lg">{otherUser.name}</Text>
              <Text className="text-gray-500 text-sm">{otherUser.online ? "Online" : "Offline"}</Text>
            </View>
          </>
        )}
      </View>
